...
- Accessing and controlling remoteproc devices typically requires root permissions, because the driver interfaces are located in the `/sys/class` directory.
- To ensure Remoteproc Runtime has the necessary privileges, run your container engine (e.g., Docker daemon, K3S, or Podman) with root privileges (typically via `sudo`). This allows it to spawn containers with Remoteproc Runtime running as root.
